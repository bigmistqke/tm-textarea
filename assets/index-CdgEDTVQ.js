...
const memoize$1 = (fn) => {
    ...
    return (...args) => {
        ...
        if (resultCached !== undefined || cache.has(id))
            return resultCached;
...
/* IMPORT */
const Highlight = globalThis.Highlight || (Set);
...
const Highlights = {
    ...
    getHighlights: (() => {
        ...
        return memoize$1((identifier, styles) => {
...
/* IMPORT */
/* MAIN */
const toHighlights = (tokens, options) => {
    const output = document.createElement('pre');
    output.classList.add(CLASS);
    output.style.backgroundColor = options?.backgroundColor || BACKGROUND_COLOR;
    const string = toString$1(tokens);
    const text = document.createTextNode(string);
    output.appendChild(text);
    if (!Highlights.isSupported())
        return [output, () => { }]; // Not supported, bailing out
    const disposers = [];
...
var mainExports$1 = main$3.exports;
...
const INITIAL = mainExports.INITIAL || main.INITIAL;
const TMRegistry = mainExports.Registry || main.Registry;
const loadOniguruma = mainExports$1.loadWASM || main$2.loadWASM;
const OnigurumaScanner = mainExports$1.OnigScanner || main$2.OnigScanner;
const OnigurumaString = mainExports$1.OnigString || main$2.OnigString;
        ...
        this.load = once(async () => {
            ...
            return {
                createOnigScanner: (patterns) => {
                    ...
                    const cached = scannersLv2.find(([cached]) => areShallowEqual(patterns, cached)); //TODO: Optimize this more maybe
                    if (cached)
                        return cached[1];
...
class Registry {
    ...
    constructor(options) {
        /* API */
        this.fetchGrammar = memoize(async (name) => {
        ...
        this.fetchTheme = memoize(async (name) => {
        ...
        this.loadGrammar = memoize(async (name) => {
            return await this.registry.loadGrammar(name) || undefined;
        });
        this.loadTheme = memoize(async (name) => {
        ...
        this.loadThemeColors = memoize(async (name) => {
        ...
        this.loadThemeColorsDefault = memoize(async (name) => {
            const theme = await this.fetchTheme(name);
        ...
        this.normalizeTheme = memoize(async (theme) => {
            theme || (theme = {
                name: 'Fallback',
                settings: []
            });
...
/* IMPORT */
/* MAIN */
const makeIntervalYielder = (interval, strategy = 'timeout') => {
...
class TextMateHighlighter {
    ...
    constructor(options) {
        /* API */
        this.highlightToAbstract = async (options, highlighter) => {
        ...
        this.highlightToANSI = (options) => {
            return this.highlightToAbstract(options, toANSI);
        };
        this.highlightToDOM = (options) => {
            return this.highlightToAbstract(options, toDOM);
        };
        this.highlightToHighlights = (options) => {
            return this.highlightToAbstract(options, toHighlights);
        };
        this.highlightToHTML = (options) => {
            return this.highlightToAbstract(options, toHTML);
        };
...
const getLeadingNewlines = (source) => source.match(/^\n+/)?.[0].length || 0;

function getLongestLineSize(input) {
...
var _tmpl$$1 = /* @__PURE__ */ template(`<div part=root><code></code><textarea part=textarea autocomplete=off inputmode=none>`), _tmpl$2$1 = /* @__PURE__ */ template(`<pre>`);
...
function createTmTextarea(styles) {
  return function TmTextarea(props) {
    ...
    const [html] = createResource(source, async (code) => `
`.repeat(getLeadingNewlines(code)) + await HIGHLIGHTER.highlightToHTML({
      code,
      grammar: props.grammar,
      theme: props.theme
    }));
    const segments = createMemo(() => html()?.replace(/<pre[^>]*>/, "").replace(/<\/pre>/, "").split("\n"));
    ...
    const max = createMemo(() => Math.floor((scrollTop() + (domRect()?.height || 0)) / props.lineHeight));
    const isVisible = createSelector(() => [min(), max()], (index, [start, end]) => index + WINDOW > start && index - WINDOW < end);
    onMount(() => {
    ...
    return (() => {
      ...
      insert(_el$2, createComponent(Index, {
        ...
        children: (line, index) => createComponent(Show, {
          get when() {
            return createMemo(() => !!line())() && isVisible(index);
          },
          get children() {
            var _el$4 = _tmpl$2$1();
            createRenderEffect((_p$) => {
              var _v$4 = styles.segment, _v$5 = line(), _v$6 = `${index * props.lineHeight}px`;
              _v$4 !== _p$.e && className(_el$4, _p$.e = _v$4);
              _v$5 !== _p$.t && (_el$4.innerHTML = _p$.t = _v$5);
              _v$6 !== _p$.a && ((_p$.a = _v$6) != null ? _el$4.style.setProperty("top", _v$6) : _el$4.style.removeProperty("top"));
              return _p$;
            }, {
              e: void 0,
              t: void 0,
      ...
      _el$3.addEventListener("scroll", (e) => {
        e.preventDefault();
        e.stopPropagation();
      });
      var _ref$2 = props.textareaRef;
      typeof _ref$2 === "function" ? use(_ref$2, _el$3) : props.textareaRef = _el$3;
      setAttribute(_el$3, "spellcheck", false);
      _el$3.addEventListener("input", (e) => {
      ...
      createRenderEffect((_p$) => {
        var _v$ = styles.segments, _v$2 = styles.textarea, _v$3 = !config.editable;
      ...
      createRenderEffect(() => _el$3.value = config.value);
      return _el$;
    })();
  };
}
...
const container = "_container_1ib4o_4";
const segments = "_segments_1ib4o_25";
const segment = "_segment_1ib4o_25";
const textarea = "_textarea_1ib4o_52";
const styles = {
	container: container,
	segments: segments,
	segment: segment,
...
const TmTextarea$1 = createTmTextarea(styles);
...
const attributesToProps = Symbol('attributesToProps');
...
const classnames = ["container","segments","segment","textarea"];
...
const App = () => {
  const [mode, setMode] = createSignal("solid");
  const [theme, setCurrentThemeName] = createSignal("light-plus");
  const [grammar, setCurrentLanguageName] = createSignal("source.tsx");
  const [fontSize, setFontSize] = createSignal(10);
  const [padding, setPadding] = createSignal(5);
  const [sourceType, setSourceType] = createSignal("large");
  const [editable, setEditable] = createSignal(true);
  const [value, setValue] = createSignal(sources[sourceType()]);
  createEffect(() => setValue(sources[sourceType()]));
  return (() => {
    var _el$ = _tmpl$(), _el$2 = _el$.firstChild, _el$3 = _el$2.firstChild, _el$4 = _el$3.nextSibling, _el$5 = _el$4.firstChild, _el$6 = _el$5.firstChild, _el$7 = _el$6.nextSibling, _el$8 = _el$5.nextSibling, _el$9 = _el$8.nextSibling, _el$10 = _el$9.firstChild, _el$11 = _el$10.nextSibling, _el$12 = _el$9.nextSibling, _el$13 = _el$12.firstChild, _el$14 = _el$13.nextSibling, _el$15 = _el$12.nextSibling, _el$16 = _el$15.nextSibling, _el$17 = _el$16.firstChild, _el$18 = _el$17.nextSibling, _el$19 = _el$16.nextSibling, _el$20 = _el$19.firstChild, _el$21 = _el$20.nextSibling, _el$22 = _el$19.nextSibling, _el$23 = _el$22.firstChild, _el$24 = _el$23.nextSibling, _el$25 = _el$22.nextSibling, _el$26 = _el$25.firstChild, _el$27 = _el$26.nextSibling, _el$28 = _el$2.nextSibling, _el$29 = _el$28.firstChild;
    _el$7.$$click = () => setMode((type) => type === "custom-element" ? "solid" : "custom-element");
    insert(_el$7, mode);
    ...
    insert(_el$14, createComponent(For, {
      ...
      children: (language) => (() => {
        var _el$31 = _tmpl$2();
        insert(_el$31, language);
        return _el$31;
      })()
    }));
    _el$18.$$input = (e) => setSourceType(e.currentTarget.value);
    ...
    _el$27.$$click = (e) => {
      setEditable((editable2) => !editable2);
    };
    insert(_el$27, () => editable() ? "enabled" : "disabled");
    _el$29.style.setProperty("resize", "both");
    _el$29.style.setProperty("height", "100px");
    _el$29.style.setProperty("width", "100px");
    _el$29.style.setProperty("overflow", "hidden");
    ...
    createRenderEffect(() => _el$18.value = sourceType());
