import { TmTextareaElement } from '../custom-element'
...
export const TabIndentation = {
  binding(element: HTMLTextAreaElement | TmTextareaElement) {
   ...
   */
  onKeyDown(event: KeyboardEvent & { currentTarget: TmTextareaElement | HTMLTextAreaElement }) {
    if (event.key === 'Tab') {
   ...
   */
  onInput(event: InputEvent & { currentTarget: TmTextareaElement | HTMLTextAreaElement }) {
  ...
  outdent(source: string, tabSize: number) {
  ...
  getLeadingWhitespace(source: string) {
    return source.match(/^\s*/)?.[0] || ''
  },

  getLineStart(value: string, position: number) {
   ...
   */
  getIndentationSegments(leadingWhitespace: string, tabSize: number) {
   ...
   */
  format(source: string, tabSize: number) {
    return source
      .split('\n')
      .map(line => {
